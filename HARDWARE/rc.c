...
u16	TIM2CH2_CAPTURE_UPVAL;	//通道2输入捕获值
u16	TIM2CH2_CAPTURE_DOWNVAL;//通道2输入捕获值

u8  TIM2CH3_CAPTURE_STA=0;	//通道3输入捕获状态		    				
u16	TIM2CH3_CAPTURE_UPVAL;	//通道3输入捕获值
u16	TIM2CH3_CAPTURE_DOWNVAL;//通道3输入捕获值

u8  TIM2CH4_CAPTURE_STA=0;	//通道4输入捕获状态		    				
u16	TIM2CH4_CAPTURE_UPVAL;	//通道4输入捕获值
u16	TIM2CH4_CAPTURE_DOWNVAL;//通道4输入捕获值

u16 RC_CH[8];

u16 CH1_MxMi[2] = {1750,1250};
u16 CH2_MxMi[2] = {1750,1250};
u16 CH3_MxMi[2] = {2000,1000};
u16 CH4_MxMi[2] = {1750,1250};
...
void TIM2_Init(void)
{
	...
	//设置NVIC中TIM4中断
	NVIC_InitStructure.NVIC_IRQChannel = TIM2_IRQn;//通道设置为TIM4
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);//GPIOB 时钟开启
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_3;
	//GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;//上拉输入
	GPIO_Init(GPIOA, &GPIO_InitStructure);//GPIOB 6 7 8 9 口配置
  ...
  TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;//向上计数模式(0->?)
	TIM_TimeBaseInit(TIM2, &TIM_TimeBaseStructure);//配置TIM4
  ...
  TIM_ICInit(TIM2, &TIM2_ICInitStructure);
	...
	TIM_ITConfig(TIM2,TIM_IT_CC3,ENABLE);//允许CC3IE捕获中断	
	TIM_ITConfig(TIM2,TIM_IT_CC4,ENABLE);//允许CC4IE捕获中断
...
void TIM2_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(TIM2, TIM_IT_CC1) == SET)//捕获1发生捕获事件
	{
		TIM_ClearITPendingBit(TIM2, TIM_IT_CC1);//清除捕获1标志位
		
		if(TIM2CH1_CAPTURE_STA == 0)//捕获到上升沿
		{
			TIM2CH1_CAPTURE_UPVAL = TIM_GetCapture1(TIM2);//获取上升沿的数据
			
			TIM2CH1_CAPTURE_STA = 1;		//标记以捕获到了上升沿
			TIM_OC1PolarityConfig(TIM2,TIM_ICPolarity_Falling);//设置为下降沿捕获
		}
		else                        //捕获到下降沿 (已经捕获到一个完整的高电平脉冲！)
		{
			TIM2CH1_CAPTURE_DOWNVAL = TIM_GetCapture1(TIM2);//获取下降沿的数据
			
			//判读是否超出发生了溢出,计算高电平脉冲时间us
			if(TIM2CH1_CAPTURE_DOWNVAL<TIM2CH1_CAPTURE_UPVAL)
			{
				RC_CH[0] = 20000 - TIM2CH1_CAPTURE_UPVAL + TIM2CH1_CAPTURE_DOWNVAL;
			}
			else
			{
				RC_CH[0] = TIM2CH1_CAPTURE_DOWNVAL- TIM2CH1_CAPTURE_UPVAL;
			}
	...
	if(TIM_GetITStatus(TIM2, TIM_IT_CC2) == SET)//捕获2发生捕获事件
	{
		TIM_ClearITPendingBit(TIM2, TIM_IT_CC2);//清除捕获2标志位
		
		if(TIM2CH2_CAPTURE_STA == 0)//捕获到上升沿
		{
			TIM2CH2_CAPTURE_UPVAL = TIM_GetCapture2(TIM2);//获取上升沿的数据
			
			TIM2CH2_CAPTURE_STA = 1;		//标记以捕获到了上升沿
			TIM_OC2PolarityConfig(TIM2,TIM_ICPolarity_Falling);//设置为下降沿捕获
		}
		else                        //捕获到下降沿 (已经捕获到一个完整的高电平脉冲！)
		{
			TIM2CH2_CAPTURE_DOWNVAL = TIM_GetCapture2(TIM2);//获取下降沿的数据
			
			//判读是否超出发生了溢出,计算高电平脉冲时间us
			if(TIM2CH2_CAPTURE_DOWNVAL<TIM2CH2_CAPTURE_UPVAL)
			{
				RC_CH[1] = 20000 - TIM2CH2_CAPTURE_UPVAL + TIM2CH2_CAPTURE_DOWNVAL;
			}
			else
			{
				RC_CH[1] = TIM2CH2_CAPTURE_DOWNVAL- TIM2CH2_CAPTURE_UPVAL;
			}
	...
	if(TIM_GetITStatus(TIM2, TIM_IT_CC3) == SET)//捕获3发生捕获事件
	{
		TIM_ClearITPendingBit(TIM2, TIM_IT_CC3);//清除捕获3标志位
		
		if(TIM2CH3_CAPTURE_STA == 0)//捕获到上升沿
		{
			TIM2CH3_CAPTURE_UPVAL = TIM_GetCapture3(TIM2);//获取上升沿的数据
			
			TIM2CH3_CAPTURE_STA = 1;		//标记以捕获到了上升沿
			TIM_OC3PolarityConfig(TIM2,TIM_ICPolarity_Falling);//设置为下降沿捕获
		}
		else                        //捕获到下降沿 (已经捕获到一个完整的高电平脉冲！)
		{
			TIM2CH3_CAPTURE_DOWNVAL = TIM_GetCapture3(TIM2);//获取下降沿的数据
			
			//判读是否超出发生了溢出,计算高电平脉冲时间us
			if(TIM2CH3_CAPTURE_DOWNVAL<TIM2CH3_CAPTURE_UPVAL)
			{
				RC_CH[2] = 20000 - TIM2CH3_CAPTURE_UPVAL + TIM2CH3_CAPTURE_DOWNVAL;
			}
			else
			{
				RC_CH[2] = TIM2CH3_CAPTURE_DOWNVAL- TIM2CH3_CAPTURE_UPVAL;
			}
	...
	if(TIM_GetITStatus(TIM2, TIM_IT_CC4) == SET)//捕获4发生捕获事件
	{
		TIM_ClearITPendingBit(TIM2, TIM_IT_CC4);//清除捕获4标志位
		
		if(TIM2CH4_CAPTURE_STA == 0)//捕获到上升沿
		{
			TIM2CH4_CAPTURE_UPVAL = TIM_GetCapture4(TIM2);//获取上升沿的数据
			
			TIM2CH4_CAPTURE_STA = 1;		//标记以捕获到了上升沿
			TIM_OC4PolarityConfig(TIM2,TIM_ICPolarity_Falling);//设置为下降沿捕获
		}
		else                        //捕获到下降沿 (已经捕获到一个完整的高电平脉冲！)
		{
			TIM2CH4_CAPTURE_DOWNVAL = TIM_GetCapture4(TIM2);//获取下降沿的数据
			
			//判读是否超出发生了溢出,计算高电平脉冲时间us
			if(TIM2CH4_CAPTURE_DOWNVAL<TIM2CH4_CAPTURE_UPVAL)
			{
				RC_CH[3] = 20000 - TIM2CH4_CAPTURE_UPVAL + TIM2CH4_CAPTURE_DOWNVAL;
			}
			else
			{
				RC_CH[3] = TIM2CH4_CAPTURE_DOWNVAL- TIM2CH4_CAPTURE_UPVAL;
			}
